...
void DrawShape::drawRectangle(Gdiplus::Graphics& graphics,
    ...
    if (rectangle->getRadius().x != 0 || rectangle->getRadius().y != 0) {
        float dx = (std::min)(rectangle->getRadius().x * 2, width);
        float dy = (std::min)(rectangle->getRadius().y * 2, height);
...
void DrawShape::drawCircle(Gdiplus::Graphics& graphics, Circle* circle) const {
    ...
    if (Gdiplus::PathGradientBrush* brush =
        ...
        if (circle->getGradient() && !circle->getGradient()->getStops().empty()) {
...
void DrawShape::drawEllipse(Gdiplus::Graphics& graphics, MyEllipse* ellipse) const {
    ...
    if (Gdiplus::PathGradientBrush* brush =
        ...
        if (ellipse->getGradient() && !ellipse->getGradient()->getStops().empty()) {
    ...
    graphics.FillEllipse(ellipse_fill, 
                        ellipse->getPosition().x - ellipse->getRadius().x,
                        ellipse->getPosition().y - ellipse->getRadius().y,
                        ellipse->getRadius().x * 2, ellipse->getRadius().y * 2);
    graphics.DrawEllipse(&ellipse_outline, 
                        ellipse->getPosition().x - ellipse->getRadius().x,
                        ellipse->getPosition().y - ellipse->getRadius().y,
                        ellipse->getRadius().x * 2, ellipse->getRadius().y * 2);
...
void DrawShape::drawPolygon(Gdiplus::Graphics& graphics, MyPolygon* polygon) const {
    ...
    if (Gdiplus::PathGradientBrush* brush =
        ...
        if (polygon->getGradient() && !polygon->getGradient()->getStops().empty()) {
...
void DrawShape::drawPolyline(Gdiplus::Graphics& graphics,
    ...
    for (size_t i = 2; i < points.size(); ++i) {
        path.AddLine(points[i - 1].x, points[i - 1].y, 
                     points[i].x, points[i].y);
    }
    // không CloseFigure() → giữ đúng polyline
    ...
    if (Gdiplus::PathGradientBrush* brush =
        ...
        if (polyline->getGradient() && !polyline->getGradient()->getStops().empty()) {
            ColorShape color = polyline->getGradient()->getStops().back().getColor();
            Gdiplus::SolidBrush corner_fill(color.toGDIColor());
            graphics.FillPath(&corner_fill, &path);
        }
    }
...
void DrawShape::drawPath(Gdiplus::Graphics& graphics, Path* path) const {
    ...
    for (int i = 0; i < n; ++i) {
        if (points[i].tc == 'm') {
            ...
            if (i + 1 < n) {
                ...
                Vector2Df auto_control_point = cur_point;
                if (i >=2 && (points[i - 1].tc == 'c' || points[i - 1].tc == 's')) {
            ...
            Vector2Df auto_control_point = cur_point;
            if (i >= 2 && (points[i - 1].tc == 'q' || points[i - 1].tc == 't')) {
                /*
            ...
            float rx = std::fabs(points[i].radius.x);
            float ry = std::fabs(points[i].radius.y);
            // If either radius is zero, treat it as a line segment
            if (rx == 0 || ry == 0) {
                gdi_path.AddLine(cur_point.x, cur_point.y, 
            ...
            float angle = x_axis_rotation * acos(-1) / 180.0f;
            float cosAngle = cos(angle);
            float sinAngle = sin(angle);

            float X = (cur_point.x - end_point.x) / 2.0f;
            float Y = (cur_point.y - end_point.y) / 2.0f;

            Vector2Df point1;
            point1.x = (cosAngle * X + sinAngle * Y);
            point1.y = (-sinAngle * X + cosAngle * Y);
            ...
            if (radii_check > 1.0f) {
                rx *= std::sqrt(radii_check);
                ry *= std::sqrt(radii_check);
            }
            ...
            Vector2Df center;
            X = (cur_point.x + end_point.x) / 2.0f;
            Y = (cur_point.y + end_point.y) / 2.0f;
            center.x = (cosAngle * point2.x - sinAngle * point2.y) + X;
            center.y = (sinAngle * point2.x + cosAngle * point2.y) + Y;
            ...
            if (sweep_flag && delta_angle < 0) {
                delta_angle += 2.0f * acos(-1);
            } else if (!sweep_flag && delta_angle > 0) {
                delta_angle -= 2.0f * acos(-1);
            }

            float start_angle_degree = start_angle * 180.0f / acos(-1); 
            float delta_angle_degree = delta_angle * 180.0f / acos(-1);
            
            // Add the elliptical arc to the path
            gdi_path.AddArc(center.x - rx, center.y - ry, 2.0f * rx, 2.0f * ry,
    ...
    if (Gdiplus::PathGradientBrush* brush =
        ...
        if (path->getGradient() && !path->getGradient()->getStops().empty()) {
            ...
            if (path->getGradient()->getUnits() == "userSpaceOnUse") {
                ...
                float r = dynamic_cast<RadialGradient*>(path->getGradient())->getRadius().x;
...
void DrawShape::drawText(Gdiplus::Graphics& graphics, Text* text) const {
    ...
    Gdiplus::Pen text_outline(outline_color.toGDIColor(), text->getOutlineThickness());
	...
	// Add the text to the graphics path
    Gdiplus::GraphicsPath path;
    path.AddString(wide_content.c_str(), (int)wide_content.length(), &font_family,
    ...
    if (auto* brush = dynamic_cast<Gdiplus::PathGradientBrush*>(text_fill)) {
        if (text->getGradient() && !text->getGradient()->getStops().empty()) {
...
void DrawShape::draw(Gdiplus::Graphics& graphics, SVGElement* shape) const {
    ...
    if (type == "group") {
        ...
        // Debug log khi gặp shape chưa hỗ trợ
        std::cerr << "[DrawShape] Unsupported shape type: " << shape->getClass() << "\n";
    }
}